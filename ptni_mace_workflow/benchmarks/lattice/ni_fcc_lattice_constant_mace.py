#!/usr/bin/env python
"""Predict the fcc Ni equilibrium lattice constant with a MACE model.
...
def maybe_plot(path: Path, rows: list[dict], a0: float, e0: float, title: str) -> bool:
    ...
    ax.set_xlabel("fcc Ni lattice constant a (A)")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scan fcc Ni lattice constants using MACE.")
    parser.add_argument("--model", type=Path, required=True, help="MACE .model file, not compiled.model.")
    parser.add_argument(
        "--scan",
        type=parse_scan,
        default=parse_scan("3.35:3.75:0.01"),
        help="Lattice constants in A. Use start:stop:step or comma list. Default: 3.35:3.75:0.01.",
    )
    parser.add_argument(
        "--fit-window",
        type=float,
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("mace_workspace/runs/benchmarks/lattice/manual/ni_fcc_lattice_test"))
    parser.add_argument("--name", default="Ni_fcc_mace")
    ...
    rows = []
    for a in args.scan:
        atoms = bulk("Ni", "fcc", a=a, cubic=True)
        if reps != (1, 1, 1):
    ...
    plotted = maybe_plot(png_path, rows, quad_a0, quad_e0, "MACE fcc Ni lattice scan") if args.plot else False

    lines = [
        "# fcc Ni Lattice Constant Test",
        "",
    ...
    lines.extend(
        [
            ...
            "- Compare against the same DFT functional/settings used in the training data, not only experimental room-temperature Ni.",
