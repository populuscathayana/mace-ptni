# 08 semi-rfKMC / MCMD 动力学雏形

本模块是 vacancy-mediated semi-rfKMC / MCMD 的早期原型。这里的 `semi-rfKMC` 可以理解为“半速率 KMC”：每步先随机提出一个低配位原子，然后对这个原子周围所有合法 He/vacancy 位点都执行显式 CI-NEB，最后在这些局部候选事件中按速率概率选择。它不是完整生产级 KMC/MD 框架，也不会默认枚举全体系所有竞争事件，而是把以下几件事串起来，方便检查 PtNi NP 或 slab 中空位介导邻位迁移的物理合理性：

1. 用 `reconstruct_close_packed_sites.py` 重构潜在 close-packed 空位位点。
2. 每步随机选择一个配位数小于 12 且存在合法邻居 He/vacancy site 的真实原子。
3. 对该原子的所有合法邻居 He/vacancy site 都生成 atom-to-vacancy hop 或 dissolution 候选。
4. 对这些局部候选事件逐个执行 ASE CI-NEB，得到显式过渡态能垒。
5. 对已评估候选计算 `nu * exp(-Ea/kBT)`，按局部速率概率选择事件并推进近似时间。
6. 默认在完成 MC-NEB hop 之后执行 ASE+MACE MD relaxation 段。

## semi-rfKMC 的含义

当前默认 `atom-random` 模式不是完整 rfKMC。完整 rfKMC/KMC 通常需要在每个 KMC step 枚举当前结构中所有合法事件，计算每个事件速率，然后按全局速率竞争选择事件。

本模块现在采用的是更轻量的 semi-rfKMC：

```text
random under-coordinated atom
  -> all legal neighboring He/vacancy sites for this atom
  -> explicit CI-NEB barrier for every local candidate
  -> local rate-weighted selection and approximate clock update
  -> accept hop or dissolution event
  -> optional MD relaxation
```

因此日志里的 `p` 是“当前随机选中原子周围局部候选事件”的速率权重。若该原子周围有多个合法 He/vacancy 位点，且多个 NEB 事件都有有效速率，`p` 就不应恒为 1；只有局部候选中只剩一个有效事件时才会是 1。这个模式更适合做路径探索、异常诊断和模型稳定性压力测试；若要提取严格扩散系数，后续应升级为完整 rfKMC/KMC 事件目录。
...
4. 对第一个有合法邻居 He 的原子，把所有合法 He site 都作为局部候选事件。
5. 对这些局部候选事件逐个计算 NEB，并按速率概率选择。
```
...
| 参数 | 默认值 | 含义 |
| --- | ---: | --- |
| `--selection-mode` | `atom-random` | 随机低配位原子，再枚举该原子的所有合法邻居 He |
...
reconstruct close-packed sites
  -> randomly choose one mobile under-coordinated atom
  -> enumerate all legal neighboring vacancy-mediated hop or dissolution trials for this atom
  -> run CI-NEB for every local candidate
  -> choose by local Arrhenius rates
...
- 这是 semi-rfKMC / MCMD 原型，不声明物理时间已经达到严格 KMC 级别。
- 默认 `atom-random` 不需要初始 vacancy；只有 `vacancy-tracked` 兼容模式才需要手动指定初始 vacancy。
