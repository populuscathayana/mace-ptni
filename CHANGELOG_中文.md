...
- MCMD 新增默认 `atom-random` 选择规则：每步随机挑选配位数 `<12` 且存在合法邻居 He/vacancy site 的真实原子，再随机挑选一个邻居 He 尝试迁移。
- 新增 Ni dissolution 事件：Ni 初始配位 `<9` 且迁移目标位配位 `<3` 时，事件标记为 `dissolution`；CI-NEB 通过后若被接受，Ni 从结构中删除而不是停留在目标 He 位点。
- `events.csv` / `mcmd_steps.csv` 新增事件类型、初始配位数和目标位配位数记录。
